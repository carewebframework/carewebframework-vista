...
public class LocationContext extends ManagedContext<Location> {
    ...
    public static Location getDefaultLocation() {
        try {
            ...
            String id = propLocation.getValue();
            return VistAUtil.validateIEN(id) ? DomainObjectFactory.get(Location.class, id) : null;
        } catch (Exception e) {
            return null;
        }
    }
    ...
    public static void setDefaultLocation(Location location) {
        try {
            Property propLocation = new Property(PROPERTY_DEFAULT_LOCATION);
            propLocation.setValue(location.getDomainId());
